...
Курсовая работа по интеграции с внешним API для получения и обработки вакансий с платформы HeadHunter и сохранения их в базу данных.
...
│   ├── __init__.py
│   └── db_manager.py      # Класс DBManager для SQLite
│
├── utils/                 # Утилиты
│   ├── __init__.py
...
8. **Загрузить вакансии компаний в БД (hh.ru → БД)**
9. **Показать компании и количество вакансий (БД)**
10. **Показать все вакансии (БД)**
11. **Показать среднюю зарплату (БД)**
12. **Показать вакансии с зарплатой выше средней (БД)**
13. **Поиск вакансий по ключевому слову (БД)**
0. Выход
...
- Вакансии для этих компаний можно загрузить из hh.ru в базу данных (SQLite).
...
- **8** — загрузить вакансии для 10 компаний в БД
...
1. **Наследование**: `HeadHunterAPI` наследуется от `BaseAPI`, `JSONSaver` наследуется от `BaseStorage`, `DBManager` — отдельный класс для работы с БД
...
- `vacancies.db` — база данных SQLite (создается автоматически)
